...
export default function Marksheet() {
  ...
  const handleEdit = async (id) => {
    ...
    setEdit(true);
    axios
      .get(`${baseUrl}/marksheet/fetch-single/${id}`)
      .then((resp) => {
        Formik.setFieldValue("msCode", resp.data.data.msCode);
        Formik.setFieldValue("name", resp.data.data.name);
        Formik.setFieldValue(
          "msDate",
          resp.data.data.msDate
            ? dayjs(resp.data.data.msDate).format("YYYY-MM-DD")
            : "",
        );
        ...
        const editMarksheetDetails = resp.data.data.marksheetDetails.map(
          (row) => ({
            ...row,
            student: allStudents.find((f) => f._id === row.student) || null,
            isEdit: true,
          }),
        );
  ...
  const handlePrint = async (id) => {
    ...
    if (user?.role === "TEACHER") {
      window.open(`/teacher/MarksheetPrint?id=${id}`, "_blank");
    } else {
      window.open(`/school/MarksheetPrint?id=${id}`, "_blank");
    }
  ...
  const Formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      const hasDuplicate =
        new Set(marksheetDetails.map((d) => d.student?._id.toString())).size !==
        marksheetDetails.length;
  ...
  const fetchPeriods = async () => {
    ...
    if (teacherId) {
      const periodsResponse = await axios.get(
        `${baseUrl}/period/teacher/${teacherId}`,
      ); // Fetch based on class
  ...
  const validateTeacher = () => {
    ...
    if (selectedClass) {
      isExists = periods.some(
        (classData) => classData._id === selectedClass?._id,
      );
    ...
    if (selectedSection) {
      isExists = periods.some(
        (sectionData) => sectionData._id === selectedSection?._id,
      );
      console.log(isExists); // true
      if (!isExists) {
        setDataError("Class-Period is not assigned to this teacher");
    ...
    if (selectedSubject) {
      isExists = periods.some(
        (subjectData) => subjectData._id === selectedSubject?._id,
      );
      console.log(isExists); // true
      if (!isExists) {
        setDataError("Class-Period is not assigned to this teacher");
                    ...
                    <Box>
                      <Autocomplete
                        ...
                        onChange={(event, newValue) => {
                          ...
                          Formik.setFieldValue("marksLimit", 0);
                        }}
                    ...
                    {studentMarksheet.map((value, i) => (
                      ...
                      >
                        ...
                        <TableCell align="right">
                          {dayjs(value.msDate).format("DD-MM-YYYY")}
                        </TableCell>

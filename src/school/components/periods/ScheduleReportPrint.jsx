  ...
  useEffect(() => {
    const fetchReportData = async () => {
      ...
      try {
        ...
        const periodsData = await axios.get(
          `${baseUrl}/schoolreports/schedule-print`,
          {
            params: params,
          },
        );
        ...
        while (currentDate <= to_Date) {
          ...
          console.log(
            `Date: ${currentDate.toISOString().slice(0, 10)}, Day: ${dayName}`,
          );
        ...
        const subjects = [
          ...new Set(
            periodsData.data.data.map((item) => item?.subjectkey.toLowerCase()),
          ),
        ];
    ...
    <>
      <Document>
        ...
        <Page size="A4" orientation="landscape" style={styles.page}>
          ...
          {/* 🔷 Title */}
          <Text style={styles.reportTitle}>Schedule/Timetable</Text>
          ...
          <View>
            ...
            <View style={styles.rowStyle}>
              ...
              <Text style={styles.valueStyle}>{selectedClass?.class_name}</Text>
            ...
            {selectedTeacher && (
              <View style={styles.rowStyle}>
                ...
                <Text style={styles.valueStyle}>{selectedTeacher?.name}</Text>
              </View>
            )}
          </View>
            ...
            {rows.map((item, index) => (
              <>
                ...
                <View style={styles.row} key={index}>
                  ...
                  {item?.subject1 && (
                    <>
                      <Text style={styles.cell}>{item.subject1}</Text>
                    </>
                  )}
                  {item?.subject2 && (
                    <>
                      <Text style={styles.cell}>{item.subject2}</Text>
                    </>
                  )}
                  {item?.subject3 && (
                    <>
                      <Text style={styles.cell}>{item.subject3}</Text>
                    </>
                  )}
                  {item?.subject4 && (
                    <>
                      <Text style={styles.cell}>{item.subject4}</Text>
                    </>
                  )}
                  {item?.subject5 && (
                    <>
                      <Text style={styles.cell}>{item.subject5}</Text>
                    </>
                  )}
                  {item?.subject6 && (
                    <>
                      <Text style={styles.cell}>{item.subject6}</Text>
                    </>
                  )}
                  {item?.subject7 && (
                    <>
                      <Text style={styles.cell}>{item.subject7}</Text>
                    </>
                  )}
                  {item?.subject8 && (
                    <>
                      <Text style={styles.cell}>{item.subject8}</Text>
                    </>
                  )}
                  {item?.subject9 && (
                    <>
                      <Text style={styles.cell}>{item.subject9}</Text>
                    </>
                  )}
                  {item?.subject10 && (
                    <>
                      <Text style={styles.cell}>{item.subject10}</Text>
                    </>
                  )}
                </View>
              </>

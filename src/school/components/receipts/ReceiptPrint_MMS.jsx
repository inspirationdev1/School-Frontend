...
export default function ReceiptPrint_MMS() {
  ...
  useEffect(() => {
    const fetchPrintReceipt = async () => {
      try {
        const receiptPrintResponse = await axios.get(
          `${baseUrl}/receipt/fetch-print/${id}`,
          { params: { id: id } },
        );
        ...
        const rptHeader = {
          school_name: receiptPrintResponse.data.data?.school.school_name,
          address: receiptPrintResponse.data.data?.school.address,
          city: receiptPrintResponse.data.data?.school.city,
          state: receiptPrintResponse.data.data?.school.state,
          country: receiptPrintResponse.data.data?.school.country,
          school_image: receiptPrintResponse.data.data?.school.school_image,
        };
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View>
          ...
          <View style={styles.rowStyle}>
            ...
            <Text style={styles.valueStyle}>
              {printReceipt.receiptDetails[0]?.parent?.name}
            </Text>

            <Text style={styles.labelStyle}>Academic year :</Text>
            <Text style={styles.valueStyle}>{printReceipt.academicyear}</Text>
          </View>

          {/* Row 3 */}
          <View style={styles.rowStyle}>
            <Text style={styles.labelStyle}>Status :</Text>
            <Text style={styles.valueStyle}>{printReceipt.status}</Text>

            <Text style={styles.labelStyle}>Mode of Pay :</Text>
            <Text style={styles.valueStyle}>{printReceipt?.paymentMethod}</Text>
          </View>
        </View>
        ...
        <Table style={styles.table}>
            ...
            <TR key={index}>
              ...
              <TD style={styles.td}>
                <Text style={styles.rightText}>
                  {formatAmount(item?.siId?.netAmount)}
                </Text>
              </TD>
            </TR>
          ))}
          ...
          <TR key={100}>
            ...
            <TD style={styles.td}>
              <Text style={styles.rightText}>
                {formatAmount(printReceipt.totalpaidAmount)}
              </Text>
            </TD>
          </TR>
        </Table>

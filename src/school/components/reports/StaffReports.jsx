...
export default function StaffReports() {
  ...
  const handlePrint = async () => {
    ...
    if (user?.role === 'STUDENT') {
        ...
        if (selectedReport.reportId === "teacher-list-report") {
          window.open(
          `/school/TeacherListReportPrint?data=${encodeURIComponent(JSON.stringify(data))}`,
          "_blank"
        );
      }else if (selectedReport.reportId === "employee-list-report") {
          window.open(
          `/school/EmployeeListReportPrint?data=${encodeURIComponent(JSON.stringify(data))}`,
          "_blank"
        );
      }
    }
  ...
  const fetchReportNames = async () => {
    try {
      const reportsData = [
      { reportId: "teacher-list-report", reportName: "Teacher-List Report" },
      { reportId: "employee-list-report", reportName: "Employee-List Report" },
      ];

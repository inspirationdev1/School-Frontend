...
import { transfercertificateSchema } from "../../../yupSchema/transfercertificateSchema";

export default function Transfercertificates() {
  const [transfercertificates, setTransfercertificates] = useState([]);
  ...
  const handleDelete = (id) => {
    if (confirm("Are you sure you want to delete?")) {
      axios
        .delete(`${baseUrl}/transfercertificate/delete/${id}`)
        .then((resp) => {
  ...
  // Filter Tab 2 results by Transfer Certificate Name, Code, Class, Section, or Student.
  const filteredTransfercertificates = transfercertificates.filter((value) => {
    const searchText = search.toLowerCase().trim();

    if (!searchText) return true;

    return (
      value.transfercertificate_name?.toLowerCase().includes(searchText) ||
      value.transfercertificate_code?.toLowerCase().includes(searchText) ||
  ...
  const handleEdit = (id) => {
    ...
    setEdit(true);
    axios
      .get(`${baseUrl}/transfercertificate/fetch-single/${id}`)
      .then((resp) => {
        Formik.setFieldValue(
          "transfercertificate_name",
          resp.data.data.transfercertificate_name,
        );
        Formik.setFieldValue(
          "transfercertificate_code",
          resp.data.data.transfercertificate_code,
        );

        Formik.setFieldValue(
          "docDate",
          resp.data.data.docDate
            ? dayjs(resp.data.data.docDate).format("YYYY-MM-DD")
            : "",
        );
        ...
        Formik.setFieldValue(
          "leaveDate",
          resp.data.data.leaveDate
            ? dayjs(resp.data.data.leaveDate).format("YYYY-MM-DD")
            : "",
        );
  ...
  const handlePrint = async (id) => {
    ...
    window.open(`/school/TransfercertificatePrint?id=${id}`, "_blank");
    setPrint(false);
  };
  ...
  const Formik = useFormik({
    ...
    validationSchema: transfercertificateSchema,
    onSubmit: (values) => {
      ...
      if (isEdit) {
        ...
        axios
          .patch(`${baseUrl}/transfercertificate/update/${editId}`, {
            ...values,
          })
          .then((resp) => {
        ...
        axios
          .post(`${baseUrl}/transfercertificate/create`, { ...values })
          .then((resp) => {
  ...
  const fetchtransfercertificates = () => {
    axios
      .get(`${baseUrl}/transfercertificate/fetch-all`)
      .then((resp) => {
        console.log("Fetching data in  Casting Calls  admin.", resp);
        setTransfercertificates(resp.data.data);
      })
      .catch((e) => {
        ...
        <Box sx={{ borderBottom: 1, borderColor: "divider", mb: 2 }}>
          ...
          >
            ...
            <Tab
              label={
                isEdit
                  ? "Edit Transfercertificate"
                  : "Add New Transfercertificate"
              }
            />
            <Tab label="View List" />
          </Tabs>
            ...
            <Paper sx={{ p: 3, m: 2 }}>
              ...
              >
                ...
                <Box>
                  <TextField
                    fullWidth
                    label="Transfercertificate Name"
                    name="transfercertificate_name"
                    value={Formik.values.transfercertificate_name}
                    onChange={Formik.handleChange}
                    onBlur={Formik.handleBlur}
                    size="small"
                  />
                  {Formik.touched.transfercertificate_name &&
                    Formik.errors.transfercertificate_name && (
                      <p style={{ color: "red" }}>
                        {Formik.errors.transfercertificate_name}
                      </p>
                    )}
                ...
                {/* Transfercertificate Code */}
                <Box>
                  <TextField
                    disabled={isEdit}
                    fullWidth
                    label="Transfercertificate Code"
                    name="transfercertificate_code"
                    value={Formik.values.transfercertificate_code}
                    onChange={Formik.handleChange}
                    onBlur={Formik.handleBlur}
                    size="small"
                  />
                  {Formik.touched.transfercertificate_code &&
                    Formik.errors.transfercertificate_code && (
                      <p style={{ color: "red" }}>
                        {Formik.errors.transfercertificate_code}
                      </p>
                    )}
          ...
          <Box>
            ...
            <Box sx={{ mb: 2 }}>
              <TextField
                ...
                placeholder="Search Name, Code, Class, Section or Student..."
                sx={{
                  "& .MuiInputBase-root": {
                    height: 42,
            ...
            <TableContainer component={Paper}>
              <Table sx={{ minWidth: 650 }} aria-label="simple table">
                ...
                <TableBody>
                  {filteredTransfercertificates.map((value) => (
                    <TableRow
                      key={value._id}
                      sx={{ "&:last-child td, &:last-child th": { border: 0 } }}
                    >
                      <TableCell component="th" scope="row">
                        {value.transfercertificate_name}
                      </TableCell>
                      <TableCell align="right">
                        {value.transfercertificate_code}
                      </TableCell>
                      <TableCell align="right">
